class DynamicGraph {
  ...
  initialise() {
    ...
    temp.timer = 0;
    temp.operation = floor(
      map(w / spring1.wn, this.minX, this.maxX, 0, this.w)
    );
  ...
  draw() {
    ...
    for (let n = 0; n < this.graphs.length - 1; n++) {
      beginShape();
      stroke(
        this.graphs[n].label[0],
        this.graphs[n].label[1],
        this.graphs[n].label[2]
      );
      strokeWeight(1);
      noFill();
    ...
    beginShape();
    stroke(
      this.graphs[this.graphs.length - 1].label[0],
      this.graphs[this.graphs.length - 1].label[1],
      this.graphs[this.graphs.length - 1].label[2]
    );
    ...
    for (let i = 0; i < j; i++) {
      vertex(
        this.x + this.graphs[this.graphs.length - 1][i][0],
        this.y - this.graphs[this.graphs.length - 1][i][1]
      );
